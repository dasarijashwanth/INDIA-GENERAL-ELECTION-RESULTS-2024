...
-- Seats Won by NDA Alliance Parties (with flexible matching)
SELECT 
    party AS Party_Name,
    won AS Seats_Won
FROM 
...
WITH RankedCandidates AS (
    SELECT 
        ...
        RANK() OVER (PARTITION BY cd.Constituency_ID ORDER BY (cd.EVM_Votes + cd.Postal_Votes) DESC) AS VoteRank
    FROM 
        constituencywise_details cd
    JOIN 
        constituencywise_results cr ON cd.Constituency_ID = cr.Constituency_ID
